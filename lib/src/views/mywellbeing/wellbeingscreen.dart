...
class _WellbeingScreenState extends State<WellbeingScreen> {
  ...
  Future<void> loadElements(core.Form form) async {
    ...
    List<core.FormElement>? result = await core.FormElementProvider().getElements(params);
  ...
  Widget build(BuildContext context) {
    if(featuresLoaded && answersLoaded && form.loadingStatus == core.LoadingStatus.ready)   loaded = true;
    ...
    if(userAnswersets.isNotEmpty){
      ...
      userAnswersets.forEach((answerset) {
        ...
        features.forEach((feature) {
          ...
          form.elements?.where((element) =>element.data?['formelementgroup'] == feature['id']).forEach((element) {
            /// Find the score of answers for the elements

            List<dynamic> answers = answerset['answers'].entries.toList();

           dynamic answer = answers.firstWhereOrNull((entry) =>entry.value['formelementid'].toString() == element.id.toString());

            if(answer!=null) {
              List options = element.data!['data'] as List;
              dynamic selectedOption = options.firstWhere((e) => e['id'] == answer.value['answer']);
        ...
        answerData.add({'date' : DateFormat('yyyy-MM-dd HH:mm:ss').parse(answerset['editdate']),'data':row,'color':colors[i++]});
        data.add(row);
    ...
    */
    List<String> featureTitles =this.features.map((feature)=>feature['title'] as String).toList();
        ...
        child: loaded ? SingleChildScrollView(
            child:Column(
                children:[
                  ...
                  if(userAnswersets.isNotEmpty) SizedBox(height:MediaQuery.of(context).size.height*0.5,child:wellbeingView(data,features: featureTitles,colors: colors)),
                  if(userAnswersets.isEmpty) Center(child:Text('Fill in the self assessment')),
          //        Center(child:Text("$answers answer times generated")),
                  if(userAnswersets.isNotEmpty) headerSheet(featureTitles),
                  ...answerData.map((e) => answerSheet(e,featureTitles)),
                  ElevatedButton(
                      onPressed: ()=>goToWidget(context,AssessmentScreen(form:form,navIndex: widget.navIndex,)),
          ...
          children: [CircularProgressIndicator(color: Colors.white10,), SizedBox(width: 10,), Text('Loading data')],
             ))
    );
  }
 Widget wellbeingView(data,{required List<Color> colors, required List<String> features}){
...
Widget headerSheet(headers){
  ...
  List<Widget> row = [Text('Answer date')];
...
Widget answerSheet(data,headers){
  ...
  return Card(child: Container(color: data['color'],child:Padding(padding:EdgeInsets.all(5),child:Row( mainAxisAlignment:MainAxisAlignment.spaceEvenly,children:row)
  ),)
  );
}
