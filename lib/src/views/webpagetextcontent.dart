...
class ContentPageView extends StatefulWidget {
  ...
  ContentPageView(this.commonname,
      {this.providedPage, this.route, this.bottomNavigationBar});
  @override
...
class _ContentPageViewState extends State<ContentPageView> {
  ...
  _setWebPage() {
    ...
    setState(() => _pageLoadingState = LoadingState.done);
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      if (widget.providedPage != null) {
        ...
        print(
            'setting provided page ' + (widget.providedPage!.pagetitle ?? ''));
        _setWebPage();
      } else {
        print('loading page ' + widget.commonname);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text(this.pages.isNotEmpty && this.pages[0].pagetitle != null
              ? this.pages[0].pagetitle!.toString()
              : AppLocalizations.of(context)!.pageContent),
          elevation: 0.1,
        ),
        ...
        if (this.pages.isEmpty)
          return Container(
            child: ListTile(
                ...
                title: Text(AppLocalizations.of(context)!.contentNotFound +
                    ' (' +
                    widget.commonname +
                    ' [' +
