...
class _RoutineScreenState extends State<RoutineScreen> {
  ...
  void load() async{
    ...
    this.routine = core.Routine.fromJson(await Provider.of<core.RoutineProvider>(context,listen: false).getDetails(widget.routine.id??0,reload:true));
    ...
    routine?.items?.forEach((element) async {
      ...
      core.RoutineItem r = core.RoutineItem.fromJson(await Provider.of<core.RoutineItemProvider>(context,listen:false).getDetails(element.id??0,reload:true));
  ...
  Widget build(BuildContext context) {
    ...
    if(kDebugMode){
      ...
      DateTime currentDate = DateUtils.dateOnly( DateTime.now());
        widgets.add(
          ElevatedButton(child: _apiClient.isProcessing ?  SizedBox(
              height: 20,
              width: 20,
            ...
            onPressed: () async {
              DateTimeRange? routineRange = await showDateRangePicker(
                  context: context,
                  firstDate: currentDate, lastDate: currentDate.add(Duration(days: 365))
              ...
              if(result['status'] == 'success'){
                ...
                Provider.of<RoutineProvider>(
                    context).getItems(
                    {},reload:true);
                showMessage(context,AppLocalizations.of(context)!.calendarUpdated ,Row(children: [Icon(Icons.check),Text(AppLocalizations.of(context)!.routineAddedToCalendar)],));
                ...
                showMessage(context, AppLocalizations.of(context)!.addingRoutineFailed,message);
              }
              setState(() {
